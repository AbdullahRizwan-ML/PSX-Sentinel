...
| `frontend/src/app/(app)/dashboard/page.tsx` | Phase 4 Session 1 — lists the 10-ticker universe, fetches `listCompanies` then per-ticker `getCompanyDetail` in parallel, plus a best-effort `getMarketSummary` strip. Designed loading-skeleton/error states, no flash-of-nothing |
| `frontend/src/app/(app)/companies/[ticker]/page.tsx` | Phase 4 Session 1 — company detail page: latest `IntelligenceReport` in full (conviction score via `ConvictionDial`, bull/bear narrative, risk factors, ML signal). Houses `MlSignalCard`, which Phase 4 Session 2 split into `MlSignalCardRich`/`MlSignalCardLegacy` (see `backend/app/schemas/intelligence.py` row below) |
| `backend/app/schemas/intelligence.py` | Pydantic v2 response/request schemas for intelligence reports, alerts, and watchlist endpoints. `IntelligenceReportResponse.score_breakdown` (Phase 4 Session 2) is hoisted out of the persisted `agent_outputs` JSON via a `@model_validator(mode="before")` — see `MlDetail`/`ScoreBreakdown` |
