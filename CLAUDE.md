...
- [ ] **Phase 2B Session 2** — IN PROGRESS / NEXT: `AnalysisOrchestrator` that wires the four agents together, builds `AgentContext` from the live database, and produces a saved `IntelligenceReport`. Wire into the existing `POST /companies/{ticker}/analyze` endpoint and the Celery `run_analysis` task (currently a placeholder). This session is also when the agents get their first real runtime test.
